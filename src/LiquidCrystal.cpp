...
void delayMicroseconds(unsigned int us)
{
    ...
    uint64_t compareValue = us * (Chip_RIT_GetBaseClock(LPC_RITIMER) / (1 * 1000 * 1000));
    Chip_RIT_Disable(LPC_RITIMER);                       // disable RIT – compare value may only be changed when RIT is disabled
    Chip_RIT_SetCompareValue(LPC_RITIMER, compareValue); // set compare value to RIT
    ...
    while (Chip_RIT_GetIntStatus(LPC_RITIMER) == false);            // wait until RIT Int flag is set
